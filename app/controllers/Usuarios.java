...
public class Usuarios extends Controller {
	...
	public static Result create() {
		...
		Usuario user = Usuario.finder.where().eq("nick", usuario.nick).findUnique();
		
		if (user != null){
			res = forbidden("Usuario "+user.nick+" ya existe");
		}
		else 
		{	
		if (usuario == null) {
	...
	public static Result update(Long uId) {
		...
		if (usuario == null) {
			...
			Usuario user = Usuario.finder.where().eq("nick", newUsuario.nick).findUnique();
			
			if (user != null){
				res = forbidden("Usuario "+user.nick+" ya existe");
			}
			else if (newUsuario == null) {
				res = badRequest(errorJson(1, "unsupported_format"));
			}
			else {
	...
	public static Result userFavs(String nombre) {
		Result res = null;
		
		Usuario usuario = Usuario.finder.where().eq("nick", nombre).findUnique();
		
		List<Favorito> lista = usuario.favs;
		res = ok(views.txt.favoritos.render(lista));
		return res;
	}
	
	public static Result userData(String nombre) {
		Result res = null;
		
		Usuario usuario = Usuario.finder.where().eq("nick", nombre).findUnique();
		
		res = ok(views.txt._usuario.render(usuario));
		return res;
	}
